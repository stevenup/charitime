datatable_json_response(json) do
  json.array! @rows do |row|
    ...
    json.actions                edit_and_del edit_admin_gyb_path(row), admin_gyb_path(row, :format => :json),
                                             {
                                               edit: { class: 'btn btn-sm btn-info edit-btn' },
                                               delete: { data: { confirm: '确认删除？' }, class: 'btn btn-sm btn-dark delete-btn m-l-sm' }
                                             }
  end
end
